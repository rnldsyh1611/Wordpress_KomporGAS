 ...
 * @extends  WP_Widget
 */
abstract class Blogprise_Widget_Base extends WP_Widget {
	...
	public function __construct() {
		...
		$this->attrs = apply_filters(
			'blogprise_widget_base_attr_arr',
			array(
				'type'          => 'text',
				'class'         => '',
				'wrapper_class' => '',
		...
		add_action( 'admin_enqueue_scripts', array( $this, 'blogprise_widget_scripts' ) );
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		foreach ( $this->settings as $key => $setting ) {
			...
			if( 'heading' == $setting['type'] || 'group' == $setting['type'] || 'subtitle' == $setting['type'] || 'message' == $setting['type'] ) {
				continue;
			}
			 ...
			 */
			$instance[ $key ] = apply_filters( 'blogprise_widget_settings_sanitize', $instance[ $key ], $new_instance, $key, $setting );
		}
	...
	public function form( $instance ) {
		...
		foreach ( $this->settings as $key => $setting ) {
			...
			switch ( $setting['type'] ) {
				...
				case 'group':
					$css = 'text-align:center;background-color:red;color:#fff;padding:8px 0;margin-top:20px;';
					if ( ! empty( $attr['css'] ) ) {
						$css = $attr['css'];
					}
					?>
					<div class="<?php echo esc_attr( $attr['wrapper_class'] ); ?>">
						<h3 class="widefat <?php echo esc_attr( $attr['class'] ); ?>" style="<?php echo esc_attr( $css ); ?>">
				...
				case 'heading':
					$css = 'text-align:center;background-color:#000;color:#fff;padding:8px 0;margin-top:20px;';
					if ( ! empty( $attr['css'] ) ) {
						$css = $attr['css'];
					}
					?>
				...
				case 'subtitle':
					$css = 'display:block;padding-bottom:10px;color:#797b7d;';
					if ( ! empty( $attr['css'] ) ) {
						$css = $attr['css'];
					}
					?>
				...
				case 'multi-checkbox':
					...
					?>
					<div class="uf-multi-checkbox <?php echo esc_attr( $attr['wrapper_class'] ); ?>">
				...
				default:
					do_action( 'blogprise_widget_field_' . $setting['type'], $key, $value, $setting, $instance );
					break;
			}
		}
	}
	 ...
	 * @return void
	 */
	public function blogprise_widget_scripts( $hook ) {
		if ( 'widgets.php' === $hook ) {
			...
			wp_enqueue_media();
			wp_enqueue_style( 'blogprise_widgets_css', get_template_directory_uri() . '/inc/widgets/widget-base/css/style' . $min . '.css' );
			wp_enqueue_script( 'blogprise_widgets', get_template_directory_uri() . '/inc/widgets/widget-base/js/script' . $min . '.js', array( 'jquery', 'wp-color-picker' ), _S_VERSION, true );
		}
	}
